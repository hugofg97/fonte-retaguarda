...
export default function PaymentCard(props) {
  ...
  return (
    <Box display="flex" flexDirection="column" marginTop={5} padding="10px">
      <Typography>
        Vimos aqui que você ainda não possui assinatura ativa, se você deseja se
        tornar assinante e caminhar conosco nesta evolução espiritual, basta
        seguir com seus dados de pagamento.
      </Typography>
      {existsCard ? (
        "Já existe um cartão associado a sua conta"
      ) : (
        <Box display="flex" flexDirection="column">
          ...
          <ModalCustom
            title={
              msgError ? "Ocorreu um erro" : "Operação efetuada com sucesso"
            }
            message={msgError}
            open={openModal}
            deny={{ action: () => setMsgError(""), label: "Fechar" }}
            setOpen={() => setMsgError("")}
          />
